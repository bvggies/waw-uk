...
def generate_speaker_card(speaker):
    """Generate HTML for a single speaker card matching WiFAI style exactly"""
    name = escape_html(speaker['name'])
    org = escape_html(speaker['org'])
    title = escape_html(speaker.get('title', ''))
    
    # Default placeholder image (no TSV data)
    picture = "https://via.placeholder.com/400x400?text=Speaker"
    
    # View Profile link (no LinkedIn data from TSV)
    linkedin_url = "#"
    link_attrs = ''
...
# Speakers from WiFAI website (extracted from https://wifai.org.uk/conference-speakers/)
wifai_speakers = [
...
if speakers_start != -1:
    ...
    if div_count == 0:
        ...
        if actual_end != -1:
            ...
            print(f"Updated speakers/index.html with {len(wifai_speakers)} speakers from WiFAI website")
        else:
...
print(f"\nRebuilt speakers page with {len(wifai_speakers)} speakers from WiFAI website")
print("No data from speakers.tsv used - fresh start from WiFAI website only")
