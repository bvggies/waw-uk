...
# All 28 speakers from WiFAI website (https://wifai.org.uk/conference-speakers/)
wifai_speakers = [
...
if speakers_start != -1:
    ...
    while div_count > 0 and search_pos < len(speakers_page_content):
        if search_pos + 4 <= len(speakers_page_content) and speakers_page_content[search_pos:search_pos+4] == '<div':
            ...
            div_count -= 1
            if div_count == 0:
                speakers_end = search_pos + 6
                break
        search_pos += 1
    else:
        # Fallback: find next </div>
        speakers_end = speakers_page_content.find('</div>', speakers_start + len('<div class="speakers-grid-new">')) + 6
    ...
    print(f"Cloned WiFAI speakers page with {len(wifai_speakers)} speakers")
    print("Maintained exact design and arrangement from WiFAI website")
else:
